...
echo '<h3>'.format_user_nick($to_user['login'], $to_id, $to_user['login'], $to_id).'<span id="encrypted_status"></span></h3>
...
<label for="sender_key_hide">Скрыть</label><input type="radio" id="sender_key_hide" name="group"> открытый ключ';
	...
	echo '<textarea readonly id="active_pubkey" style="display:none;">'.$_SESSION['myuser_pubkey'].'</textarea>';
	echo '<textarea readonly id="current_pubkey" style="display:none;"></textarea>';
    }
