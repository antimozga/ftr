...
?>
<p>Запись была удалена из базы данных.</p>
<?php
	    }
	}
	...
	if (($cmd == "Добавить") || ($cmd == "Сохранить") || ($cmd == "Искать") || ($cmd == "edit")) {
	    ...
	    if ($cmd == "Добавить") {
		  ...
		  $database->exec($query);
?>
<p><b>Новая запись добавлена в базу!</b></p>
<?php
	    }
...
?>
<p>Результаты поиска в базе (<a href="groups.php">Показать всю</a>)</p>
<?php
	    }
...
<h3><?php if ($edit) echo 'Правка записи'; else echo 'Ввод новых данных или поиск по базе' ?></h3>
...
<tr><td><input type="submit" name="cmd" value=<?php if ($edit) echo '"Отменить"'; else echo '"Искать"' ?>></td><td><input type="submit" name="cmd" value=<?php if ($edit) echo '"Сохранить"'; else echo '"Добавить"' ?>></td><tr>
</table>
</form>
