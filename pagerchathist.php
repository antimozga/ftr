...
if (! $database) {
    ...
    if (is_defined('to')) {
        ...
        $user_query = "SELECT id_user, id_from_user, ForumPager.new AS new, " . "ForumPager.time AS time, ForumPager.subj AS subj, " . "ForumPager.post AS post, ForumPager.encrypted AS encrypted " . "FROM ForumUsers, ForumPager WHERE ForumUsers.id = $to_id $where_time AND" . "((ForumPager.id_from_user = $to_id AND ForumPager.id_user = $myuser_id) OR " . " (ForumPager.id_from_user = $myuser_id AND ForumPager.id_user = $to_id)) ORDER BY ForumPager.time DESC $msg_limit;";
        ...
        $user_query = "UPDATE ForumPager SET new = 0 WHERE new = 1 AND " . "(id_from_user = $to_id AND id_user = $myuser_id);";
