...
$query = "CREATE TABLE IF NOT EXISTS NewsVTomske " . "(id INTEGER PRIMARY KEY," . " time INTEGER," . " link NVARCHAR," . " id_topic INTEGER)";
$database->exec($query);
...
function addPost($post)
{
    ...
    if ($class_title !== false) {
        ...
        if (strpos($txt_date, 'Сегодня') !== false) {} else if (strpos($txt_date, 'Вчера') !== false) {
        ...
        if ($title_class !== false) {
            ...
            if ($class_author !== false) {
                ...
                if ($class_text !== false) {
                    ...
                    $database->exec("INSERT INTO ForumTopics (id, id_grp, id_user, nick, topic, id_session)" . " VALUES(NULL, $FORUM_NEWSVTOMSKE_GID, 0, '$author', '$title', '$id_session')");
                    $id_topic = $database->lastInsertId();

                    $database->exec("INSERT INTO ForumPosts (id, time, id_grp, id_topic, id_user, nick, subj, post, id_session)" . " VALUES (NULL, $tim, $FORUM_NEWSVTOMSKE_GID, $id_topic, 0, '$author', '$title', '$text', '$id_session');");

                    $database->exec("INSERT INTO NewsVTomske (id, time, link, id_topic) " . "VALUES(NULL, $tim, '$href', $id_topic)");
                    return true;
                }
            }
        }
